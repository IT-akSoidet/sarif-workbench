...
class Provenance(BaseModel):
    repo: str
    # Имя проекта задано человеком (`enrich --project`), а не выведено из
    # имени каталога. Сервер по этому признаку решает, можно ли предпочесть
    # имя проекта из самого отчёта: выведенное имя — догадка по месту
    # запуска, явное — решение.
    #
    # Необязательное со значением по умолчанию: старые sidecar читаются без
    # изменений, поднимать версию схемы не нужно.
